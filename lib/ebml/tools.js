...
var tools = {
    ...
    convertDataToType: function(data, type) {
        const UNIX_EPOCH_DELAY = tools.UNIX_EPOCH_DELAY;
        ...
        switch(type) {
            ...
            case "i":
                ...
                break;
            case "d":
                value = tools.readUnsigned(data) + UNIX_EPOCH_DELAY*tools.DATE_SCALE;
                break;
            case "s":
    ...
    readDataFromTag: function(tagObj, data) {
        ...
        if(!tools.LAZY_CONVERSION)
            tagObj.value = tools.convertDataToType(data, tagObj.type);
        return tagObj;
    },
    ...
    writeDataToTag: function(tagObj, value) {
        ...
        switch(tagObj.type) {
            case "u":
                tagObj.data = tools.encodeInteger(value);
                break;
            case "f":
                tagObj.data = tools.encodeFloat(value);
                break;
            case "i":
                tagObj.data = tools.encodeInteger(value);
                break;
            case "d":
            	if(value instanceof Date)
            		value = (+value)*tools.DATE_SCALE; //If it is js date we need to make nanoseconds from milliseconds
            	else if(BN.isBN(value))
            		value = value.subn(UNIX_EPOCH_DELAY*tools.DATE_SCALE);
            	else
            		value = (+value) - UNIX_EPOCH_DELAY*tools.DATE_SCALE;
                tagObj.data = tools.encodeInteger(value);
                break;
            case "s":
            case "8":
                tagObj.data = Buffer.from("" + value);
                break;
            case "b":
                tagObj.data = Buffer.isBuffer(value) ? value : Buffer.from(value);
                break;
            default:
    ...
    UNIX_EPOCH_DELAY: 978307200000, //In milliseconds,
